...
export function CompanyList({ companies, availablePoints }: { companies: Company[]; availablePoints: number }) {
  ...
  const filtered = useMemo(() => companies.filter((company) => {
    ...
    const matchesStatus = status === "all"
      || (status === "upcoming" && isUpcomingCompany(company.status))
  ...
  return (
    <>
      <div className="filter-bar">
        ...
        <label className="select-field"><SlidersHorizontal size={17} /><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All statuses</option><option value="upcoming">Upcoming companies</option><option value="registration_open">Registration open only</option><option value="open">Bidding now</option><option value="paused">Paused</option><option value="finalized">Finalized</option></select></label>
