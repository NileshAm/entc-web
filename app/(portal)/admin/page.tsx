    ...
    <div className="dashboard-page">
      <div className="page-title-row">
        <div>
          ...
          <p>Increase the current bid, monitor responses, and finalize eligible sessions.</p>
        </div>
      ...
      <section className="metric-grid">
        ...
        <article className="metric-card"><span className="metric-icon amber"><Hourglass /></span><div><small>PENDING RESPONSES</small><strong>{live?.pending_count ?? 0}</strong><p>Waiting for Stay or Withdraw</p></div></article>
        <article className="metric-card"><span className="metric-icon purple"><CircleDollarSign /></span><div><small>CURRENT BID</small><strong>{live?.current_bid ?? 0}</strong><p>Controlled by administrators</p></div></article>
      </section>

      {live ? (
        <>
          {live.applicant_count > live.cv_requirement && (
          ...
          {live.pending_count > 0 && (
            <div className="warning-banner"><Hourglass /><span><strong>Responses required</strong>{live.pending_count} students still need to choose Stay or Withdraw before this round can finish.</span></div>
          )}
          <section className="admin-live-card">
            <div className="admin-live-head">
              <div className="admin-live-company"><CompanyAvatar name={live.name} logoUrl={live.logo_url} large /><div><span className="live-label"><span className="live-dot" /> LIVE SESSION</span><h2>{live.name}</h2><p>{live.industry} · {live.location}</p></div></div>
            ...
            <div className="admin-live-stats">
              ...
              <div><small>CURRENT BID</small><strong>{live.current_bid}</strong><span>+{live.bid_increment} default increment</span></div>
              <div><small>RESPONSES</small><strong>{live.confirmed_count}</strong><span>{live.pending_count} still pending</span></div>
            </div>
