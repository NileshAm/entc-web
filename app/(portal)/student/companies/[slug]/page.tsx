    ...
    <div className="dashboard-page company-detail-page">
      ...
      <div className="company-detail-layout">
        <div className="company-detail-main">
          ...
          <section className="dashboard-section demand-explainer"><span className="page-kicker">LIVE DEMAND</span><h2>{demandCategory(company.demand_ratio)}</h2><div className="demand-track large"><span className={company.demand_ratio > 1 ? "over" : ""} style={{ width: `${Math.min(100, company.demand_ratio * 100)}%` }} /></div><p>{company.applicant_count} students are currently staying in the session for {company.cv_requirement} CV slots. Their names and response states are shown in the session card.</p></section>
        </div>
