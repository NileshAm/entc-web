    ...
    <div className="dashboard-page company-detail-page">
      ...
      <div className="company-detail-layout">
        <div className="company-detail-main">
          ...
          <section className="dashboard-section demand-explainer"><span className="page-kicker">LIVE DEMAND</span><h2>{demandCategory(company.demand_ratio)}</h2><div className="demand-track large"><span className={company.demand_ratio > 1 ? "over" : ""} style={{ width: `${Math.min(100, company.demand_ratio * 100)}%` }} /></div><p>{company.status === "registration_open" ? company.bidding_mode === "automatic" ? `${company.applicant_count} students have placed initial bids before live bidding for ${company.cv_requirement} CV slots. This cohort locks when bidding starts, and registration-phase withdrawals are free.` : `${company.applicant_count} students have joined before bidding for ${company.cv_requirement} CV slots. This cohort locks when bidding starts, and registration-phase withdrawals are free.` : company.bidding_mode === "automatic" ? `${company.applicant_count} students are ranked by their own bids for ${company.cv_requirement} CV slots. The top bids at closing are selected, with earlier bids winning ties.` : `${company.applicant_count} students are currently staying in the session for ${company.cv_requirement} CV slots. The committee controls increases and each participating student can stay or withdraw.`} Names and current bidding states are shown in the session card.</p></section>
        </div>
